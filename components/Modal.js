...
function Modal() {
  ...
  const uploadPost = async () => {
    ...
    //1. Create a post and add to firesotr 'posts' collection
    //2. Get the post ID for the newly created post
    //3. Upload the image to firebase storage with the post ID
    //4. Get a download URL from db storage adn update the original post with image
    ...
    <Transition.Root show={open} as={Fragment}>
      ...
      >
        <div className="flex items-end justify-center md:min-h-[800px] sm:min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
          ...
          <span
            className="hidden sm:inline-blcok sm:align-middle sm:h-screen"
            aria-hidden="true"
          >
            &#0203;
          </span>
